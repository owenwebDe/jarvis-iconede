...
class OpenAILLM(
    ...
    async def _process_stream_manual(
        ...
        if tool_call_started:
            ...
            self.logger.debug(
                "Tool call streaming finished with started tool items",
                data={
    ...
    async def _openai_completion(
        ...
        model_name = ModelDatabase.resolve_wire_model_name(
        ...
        available_tools: list[ChatCompletionToolParam] | None = cast(
            ...
            [
                ...
                if tool.name  # Skip tools without names (prevents Groq 400 errors)
            ],
        )
    ...
    def _prepare_api_request(
        ...
        wire_model = ModelDatabase.resolve_wire_model_name(
            provider=self.provider,
        ...
        max_tokens_val = request_params.maxTokens
        if self.provider in (Provider.GROQ, Provider.OPENROUTER):
            if not max_tokens_val or max_tokens_val > 4096:
                max_tokens_val = 4096
        ...
        if self._reasoning:
            ...
            base_args["max_tokens"] = max_tokens_val
            if tools:
                base_args["parallel_tool_calls"] = request_params.parallel_tool_calls
                base_args["tool_choice"] = getattr(request_params, "tool_choice", None) or "auto"
        ...
        # Temporary token budget audit — log component sizes
        try:
            import json as _json
            _sys_msg = next((m for m in (messages or []) if m.get('role') == 'system'), {})
            _sys_len = len(_sys_msg.get('content', ''))
            _hist_msgs = [m for m in (messages or []) if m.get('role') != 'system']
            _hist_len = sum(len(m.get('content', '')) if isinstance(m.get('content'), str) else len(_json.dumps(m.get('content', ''))) for m in _hist_msgs)
            _tools_len = len(_json.dumps(tools)) if tools else 0
            _total = _sys_len + _hist_len + _tools_len
            import logging as _log
            _log.getLogger('token_audit').warning(
                '[TOKEN_AUDIT_DETAIL] System: %d chars (~%d tok) | History: %d chars (~%d tok) | Tools: %d chars (~%d tok) | TOTAL: %d chars (~%d tok)',
                _sys_len, _sys_len // 4, _hist_len, _hist_len // 4, _tools_len, _tools_len // 4, _total, _total // 4
            )
        except Exception:
            pass
        return arguments
