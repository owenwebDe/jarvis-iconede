...
def create_transport_context(
    ...
    *,
    ...
    if config.transport == "stdio":
        ...
        if resolved_command in ("python", "python3", "py"):
            import sys
        ...
        *,
        ...
        def transport_context_factory():
            ...
            if config.transport == "stdio":
                ...
                if "PYTHONPATH" not in _parent_extras:
                    _parent_extras["PYTHONPATH"] = _os.pathsep.join([p for p in _sys.path if p])

                resolved_command = config.command
                if resolved_command in ("python", "python3", "py"):
                    resolved_command = _sys.executable
                ...
                server_params = StdioServerParameters(
                    ...
                    env=build_child_environment(
                        ...
                        base=get_default_environment(),
                        overrides={**_parent_extras, **_config_env},
                    ),
                    cwd=config.cwd,
                )
